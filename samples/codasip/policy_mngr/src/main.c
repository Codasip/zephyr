...
int main( void )
{
    ...
    printk( "NOTE: THIS TEST CODE ONLY WORKS IF AUX GPIO01 (Aux) 3 & 4 CONNECTIONS TO THE POLICY MANAGER ALARMS 1 & 2 HAVE BEEN CONNECTED IN THE RTL\n" );
    ...
    // gpio_pin_set_dt( &alarm1, 0 ); done in the callback

    k_sleep(K_MSEC(3000));

    printk( "Firing alarm2 \n" );
    gpio_pin_set_dt( &alarm2, 1 );
    // gpio_pin_set_dt( &alarm2, 0 ); done in the callback

    k_sleep(K_MSEC(3000));

    printk( "Setting Alarm1 for Int Callback\n" );
    
    if ( policy_mngr_set_alarm( policy_mngr_dev, POLICY_MNGR_ALARM1_GPIO3, POLICY_MNGR_ALARM_ACTION_INT_CALLBACK ) != 0 )
    {
        printk( "%s: Error setting alarm id %d\n", policy_mngr_dev->name, POLICY_MNGR_ALARM1_GPIO3 );
        return 0;
    }
    ...
    if ( policy_mngr_set_alarm( policy_mngr_dev, POLICY_MNGR_ALARM2_GPIO4, POLICY_MNGR_ALARM_ACTION_INT_CALLBACK ) != 0 )
    {
        printk( "%s: Error setting alarm id %d\n", policy_mngr_dev->name, POLICY_MNGR_ALARM2_GPIO4 );
        return 0;
    }
    ...
    if ( policy_mngr_set_alarm( policy_mngr_dev, POLICY_MNGR_ALARM2_GPIO4, POLICY_MNGR_ALARM_ACTION_RESET ) != 0 )
    {
        printk( "%s: Error setting alarm id %d\n", policy_mngr_dev->name, POLICY_MNGR_ALARM2_GPIO4 );
        return 0;
    }
