...
void ecb_mode(const struct device *dev)
{
        ...
        uint8_t ecb_key[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                               0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
        uint8_t ecb_plaintext[16] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
                                     0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
        const uint8_t ecb_ciphertext[16] = {0x69, 0xC4, 0xE0, 0xD8, 0x6A, 0x7B, 0x04, 0x30,
...
static uint8_t fake_key[16] = {0x07, 0x1b, 0x11, 0x3b, 0x0c, 0xa7, 0x43, 0xfe,
                               0xcc, 0xcf, 0x3d, 0x05, 0x1f, 0x73, 0x73, 0x82};
static uint8_t fake_nonce[12] = {0xf0, 0x76, 0x1e, 0x8d, 0xcd, 0x3d,
                                 0x00, 0x01, 0x76, 0xd4, 0x57, 0xed};
static uint8_t fake_hdr[16] = {0xe2, 0x01, 0x06, 0xd7, 0xcd, 0x0d, 0xf0, 0x76,
                               0x1e, 0x8d, 0xcd, 0x3d, 0x88, 0xe5, 0x4c, 0x2a};
static uint8_t fake_data[64] = {0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                                0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22,
                                0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d,
                                0x2e, 0x2f, 0x30, 0x13, 0xb4, 0xc7, 0x2b, 0x38, 0x9d, 0xc5, 0x01,
                                0x8e, 0x72, 0xa1, 0x71, 0xed, 0x09, 0xa4, 0x25, 0xcd, 0x9b, 0x2e,
                                0x1c, 0x9b, 0x72, 0xee, 0xe7, 0x31, 0x32, 0x33, 0x34};
static const uint8_t fake_expected[68] = {
        0x1d, 0x6c, 0x06, 0x71, 0x1d, 0x85, 0xa0, 0x9d, 0xb1, 0x80, 0xb0, 0xcd, 0x89, 0x97, 0x35,
        0x8f, 0x00, 0x72, 0x19, 0x51, 0x3c, 0xa7, 0x83, 0xb9, 0x94, 0xa6, 0x97, 0xe5, 0xa0, 0xbd,
        0x1e, 0xa3, 0x2d, 0x5c, 0x36, 0x61, 0x2f, 0x13, 0x44, 0x92, 0xac, 0x3b, 0x52, 0xe4, 0x2e,
        0xcf, 0xbf, 0xd2, 0xc0, 0x55, 0x92, 0x44, 0xe2, 0x88, 0x6a, 0x8e, 0x37, 0x49, 0xbc, 0x03,
        0x1f, 0xfd, 0x8c, 0xe6,
...
void fake_mode(const struct device *dev)
{
        ...
        if (cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES,
                                 DT_BINDINGS_CRYPTO_CIPHER_MODE_FAKE, CRYPTO_CIPHER_OP_ENCRYPT)) {
                return;
        }
        ...
        if (memcmp(encrypt.out_buf, fake_expected, sizeof(fake_expected))) {
                LOG_ERR("FAKE mode ENCRYPT - Mismatch between expected "
                        "and returned cipher text");
                print_buffer_comparison(fake_expected, encrypt.out_buf, sizeof(fake_expected));
                goto out;
        }
        ...
        if (cipher_begin_session(dev, &ini, CRYPTO_CIPHER_ALGO_AES,
                                 DT_BINDINGS_CRYPTO_CIPHER_MODE_FAKE, CRYPTO_CIPHER_OP_DECRYPT)) {
                return;
        }
        ...
        if (memcmp(decrypt.out_buf, fake_data, sizeof(fake_data))) {
                LOG_ERR("FAKE mode DECRYPT - Mismatch between plaintext "
                        "and decrypted cipher text");
                print_buffer_comparison(fake_data, decrypt.out_buf, sizeof(fake_data));
                goto out;
        }
