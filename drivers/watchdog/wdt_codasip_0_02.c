...
static int wdt_codasip_setup(const struct device *dev, uint8_t options) { /* options are ignored */
        ...
        config->policy_mgr->WDTIMER =
                data->wdtimer_feed_value; /* Writing a non-zero value starts the watchdog */
...
static int wdt_codasip_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
        const struct wdt_codasip_cfg *config = dev->config;
        struct wdt_codasip_data      *data   = dev->data;
        ...
        installed_timeout = cfg->window.max * (uint64_t)config->clock /
                            1000; /* WDT clock frequency (in Hz) from DT */
...
#define CODASIP_WDT_INIT(index)                                                                    \
        ...
        static struct wdt_codasip_data wdt_codasip_data_##index;                                   \
                                                                                                   \
        DEVICE_DT_INST_DEFINE(index, &wdt_codasip_init, NULL, &wdt_codasip_data_##index,           \
                              &wdt_codasip_cfg_##index, POST_KERNEL,                               \
                              CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &wdt_codasip_driver_api);
