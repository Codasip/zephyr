...
static int gpio_l31fpga_configure(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
        const gpio_l31fpga_config_t *cfg     = dev->config;
        gpio_l31fpga_data_t         *context = dev->data;
        l31fpga_gpio_regs_t         *base    = cfg->base; /* GPIO Base address */
        uint32_t port_map                    = cfg->port_map; /* GPIO Valid IO bits, for each
                                                                 bit: 1 = GPIO, 0 = not GPIO */
...
static int gpio_l31fpga_port_get_raw(const struct device *dev, uint32_t *value)
{
        const gpio_l31fpga_config_t *cfg  = dev->config;
        l31fpga_gpio_regs_t         *base = cfg->base; /* GPIO Base address */
...
static int gpio_l31fpga_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
        const gpio_l31fpga_config_t *cfg     = dev->config;
        gpio_l31fpga_data_t         *context = dev->data;
        l31fpga_gpio_regs_t         *base    = cfg->base; /* GPIO Base address */
...
static int gpio_l31fpga_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
        const gpio_l31fpga_config_t *cfg     = dev->config;
        gpio_l31fpga_data_t         *context = dev->data;
        l31fpga_gpio_regs_t         *base    = cfg->base; /* GPIO Base address */
...
static int gpio_l31fpga_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
        const gpio_l31fpga_config_t *cfg     = dev->config;
        gpio_l31fpga_data_t         *context = dev->data;
        l31fpga_gpio_regs_t         *base    = cfg->base; /* GPIO Base address */
...
static int gpio_l31fpga_port_toggle_bits(const struct device *dev, uint32_t mask)
{
        const gpio_l31fpga_config_t *cfg     = dev->config;
        gpio_l31fpga_data_t         *context = dev->data;
        l31fpga_gpio_regs_t         *base    = cfg->base; /* GPIO Base address */
...
static int gpio_l31fpga_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
...
static int gpio_l31fpga_manage_callback(const struct device *dev, struct gpio_callback *callback,
                                        bool set)
{
        /* No L31 Helium GPIO IRQs */
        return -ENOTSUP;
...
#define L31FPGA_GPIO_DEVICE(n)                                                                     \
        ...
        static const gpio_l31fpga_config_t gpio_l31fpga_port_##n##_config = {                      \
                ...
                .config_func = port_##n##_l31fpga_config_func,                                     \
                                                                                                   ...
                                                                                                   \
        DEVICE_DT_INST_DEFINE(n, gpio_l31fpga_init, NULL, &port_##n##_l31fpga_runtime,             \
                              &gpio_l31fpga_port_##n##_config, POST_KERNEL,                        \
                              CONFIG_GPIO_INIT_PRIORITY, &gpio_l31fpga_driver_api);                \
                                                                                                   \
        static void port_##n##_l31fpga_config_func(const struct device *dev)                       \
        {                                                                                          \
