...
#define SD_FAST_CLK_EN_NODE  DT_ALIAS(sd_fast_clk_en)
    ...
    {
        LOG_ERR("gpio_is_ready_dt(&sd_fast_clk_en) failed!");
    }

    if ( gpio_pin_configure_dt(&sd_fast_clk_en, GPIO_OUTPUT_ACTIVE) < 0 )
    {
        LOG_ERR("gpio_pin_configure_dt(&sd_fast_clk_en, GPIO_OUTPUT_ACTIVE) failed!");
    }

    printk( "spi_cfg->frequency = %d requested\n", spi_cfg->frequency );

    if ( spi_cfg->frequency <= 400000 /*SDMMC_CLOCK_400KHZ*/ )
    {
        printk( "Switching SD Clock to slow clock\n" );
        if ( gpio_pin_set_dt( &sd_fast_clk_en, 0 ) < 0 )
        {
            LOG_ERR("gpio_pin_set_dt( &sd_fast_clk_en, 0 ) failed!");
        }
    }
    else
    {
        printk( "Switching SD Clock to fast clock\n" );
        if ( gpio_pin_set_dt( &sd_fast_clk_en, 1 ) < 0 )
        {
            LOG_ERR("gpio_pin_set_dt( &sd_fast_clk_en, 1 ) failed!");
        }
    }
#endif
